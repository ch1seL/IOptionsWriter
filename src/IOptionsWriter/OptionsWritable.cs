...
namespace IOptionsWriter
{
	public class OptionsWritable<T> : IOptionsWritable<T> where T : class, new()
	{
		...
		{
			var path = _environment.ContentRootFileProvider.GetFileInfo(_settingsFile).PhysicalPath;

			var jObject = await File.OpenRead(path).ToObject<JObject>();
			...
			await File.Create(path).FromObject(jObject,
